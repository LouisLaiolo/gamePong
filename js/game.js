var game = {
  ...
  ball : {
    ...
    collide : function(anotherItem) {
      if ( !( this.sprite.posX >= anotherItem.sprite.posX + anotherItem.sprite.width || this.sprite.posX <= anotherItem.sprite.posX - this.sprite.width || this.sprite.posY >= anotherItem.sprite.posY + anotherItem.sprite.height || this.sprite.posY <= anotherItem.sprite.posY - this.sprite.height ) ) {
        return true;
      } 
      return false;
    },
  ...
  init : function() {
  	...
  	// Cibler et Redimensionner la div principale - Non important
  	this.blocToCenter = document.getElementById("blocToCenter");
	this.blocToCenter.style.width = game.conf.BLOCCENTERWIDTH + "px";
	this.blocToCenter.style.height = game.conf.BLOCCENTERHEIGHT + "px";
	this.blocToCenter.style.margin = "-" + game.conf.BLOCCENTERHEIGHT/2 + "px 0 0 -" + game.conf.BLOCCENTERWIDTH/2 + "px";
    ...
    this.groundLayer = game.display.createLayer("terrain", game.conf.GROUNDLAYERWIDTH, game.conf.GROUNDLAYERHEIGHT, this.divGame, 0, this.groundColor, 10, 50); 
    ...
    this.scoreLayer = game.display.createLayer("score", game.conf.GROUNDLAYERWIDTH, game.conf.GROUNDLAYERHEIGHT, this.divGame, 1, undefined, 10, 50);
    this.displayScore(0,0);

    // Création et affichage de la balle (associer l'objet déclaré plus haut avec un sprite)
    this.playersBallLayer = game.display.createLayer("joueursetballe", game.conf.GROUNDLAYERWIDTH, game.conf.GROUNDLAYERHEIGHT, this.divGame, 2, undefined, 10, 50);  
  ...
  initStartGameButton : function() { // Bouton start = Action de la fonction associée dans game.control
    this.startGameButton.onclick = game.control.onStartGameClickButton;
  },

  initPauseGameButton : function() { // Bouton pause = Action de la fonction associée dans game.control
    this.pauseGameButton.onclick = game.control.onPauseGameClickButton;
  },
 
  initResumeGameButton : function() { // Bouton resume = Action de la fonction associée dans game.control
    this.resumeGameButton.onclick = game.control.onResumeGameClickButton;
  },

  updateInfoGame : function() { //Vérifie l'état de la partie et modifie le contenu de la div correspondante
  	if(!this.gameOn && !this.ball.inGame){
  		if(this.playerOne.victory)
    		this.divInfo.innerHTML = "[ <b>FIN DE PARTIE - victoire de "+this.playerOne.name+"</b> ]";
    	if(this.playerTwo.victory)
    		this.divInfo.innerHTML = "[ <b>FIN DE PARTIE - victoire de "+this.playerTwo.name+"</b> ]";
  ...
  displayScore : function(scorePlayer1, scorePlayer2) {
   game.display.drawTextInLayer(this.scoreLayer, scorePlayer1, game.conf.SCOREFONTSIZE + "pt Courier New", "#AA9A8B", game.conf.SCOREPOSXPLAYER1, game.conf.SCOREPOSYPLAYER1);
   game.display.drawTextInLayer(this.scoreLayer, scorePlayer2, game.conf.SCOREFONTSIZE + "pt Courier New", "#AA9A8B", game.conf.SCOREPOSXPLAYER2, game.conf.SCOREPOSYPLAYER2);
   game.display.drawTextInLayer(this.scoreLayer , "VITESSE BALLE : " + this.ball.speed, "10px Courier New", "#AA9A8B", 10, 10);
  },
  ...
  moveBall : function() { //4) Encapsuler le mouvement, le rebond, et le refresh de l'affichage dans une seule fonction de l'objet game
  if(this.gameOn){
    this.ball.move();
    this.ball.bounce();
   }
  ...
  ballOnPlayer : function(player, ball) {
    ...
    } else if ( ball.sprite.posY >= player.sprite.posY + playerPositions*2 && ball.sprite.posY < player.sprite.posY + player.sprite.height - playerPositions ) {
      returnValue = "MIDDLEBOTTOM";
    } else if ( ball.sprite.posY >= player.sprite.posY + player.sprite.height - playerPositions && ball.sprite.posY < player.sprite.posY + player.sprite.height ) {
      returnValue = "BOTTOM";
    }
    return returnValue;
  },
  ...
  clearLayer : function(targetLayer) { //Pour jarter la trainée de la balle (voir game.display.js)
    targetLayer.clear();
  },
  ...
  movePlayers : function() {
    ...
    if ( up && game.playerOne.sprite.posY > 0 ) //Empêcher la raquette non IA de dépasser les limites du terrain de jeu
  ...
  collideBallWithPlayersAndAction : function() { //Collisions entre raquette et balle = Son + Accélérer vitesse de la balle + 0.25
    if ( this.ball.collide(game.playerOne) ) {
	  this.playerOneSound.play();
      this.changeBallPath(game.playerOne, game.ball);
      if(this.ball.speed <= this.maxGameSpeed) //La vitesse de la balle arrête d'augmenter au delà d'une certaine limite
      	this.speedUpBall();
    }
  ...
  lostBall : function() {
    if ( this.ball.lost( this.playerOne )) {
	  ...
	  if( this.playerTwo.score != this.ultimateReward ){
          ...
          setTimeout( function() { game.ai.startBall(); }, 3000 );
        }
